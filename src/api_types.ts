 ...
 *
 * This is a discriminated union of possible invalid or missing page objects.
 * Its shape is approximately equivalent to:
 * ```ts
 * {
 *   title?: string;
 *   ns?: number;
 ...
 * @private
 */
export type _PageSetInvalidTitlesAndRevisions =
	| _PageSetInvalidTitlesAndReasons
	| _PageSetInvalidSpecialPages
	| _PageSetMissingPageIDs
	| _PageSetMissingRevisionIDs
	| _PageSetMissingTitles
	| _PageSetInterwikiTitle;
/**
...
/**
* Generated by `ApiPageSet::getInterwikiTitles`.
* @private
*/
export interface _PageSetInterwikiTitle {
	title: string;
	iw: string;
	url?: string;
}
...
export type ApiResponsePurge =  // Fully checked (source code level)
	| {
		title: string; // Defined without calling ApiQueryBase::addTitleInfo
		ns: number; // Same as above
		purged?: true;
		linkupdate?: true;
	}
	| _PageSetInvalidTitlesAndRevisions;
