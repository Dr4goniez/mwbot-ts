...
/**
 * Documentation reference returned with API errors.
 *
 * Usually contains a link to the relevant MediaWiki API documentation.
 */
export type ApiDocRef = string;
...
export interface ApiResponse {
	...
	/**
	 * Present in error responses when `errorformat=bc`.
	 */
	error?: ApiResponseError;
	/**
	 * Present in error responses when `errorformat` is **not** `bc`.
	 */
	errors?: ApiResponseErrors[];
	/**
	 * Present in error responses when `formatversion=1` and `errorformat` is **not** `bc`.
	 */
	'*'?: ApiDocRef;
	/**
	 * Present in error responses when `formatversion=2` and `errorformat` is **not** `bc`.
	 */
	docref?: ApiDocRef;
	errorlang?: string;
...
export interface ApiResponseError {
	...
	info: string;
	/**
	 * Present when `formatversion=1`.
	 */
	'*'?: ApiDocRef;
	/**
	 * Present when `formatversion=2`.
	 */
	docref?: ApiDocRef;
}
...
export interface ApiResponseErrors {
	...
	module: string;
	/**
	 * Present when `formatversion=1` and `errorformat` is either `plaintext`,
	 * `wikitext`, or `html`.
	 */
	'*'?: string;
	/**
	 * Present when `errorformat=raw` (for both `formatversion=1` and `2`).
	 */
	key?: string;
	/**
	 * Present when `errorformat=raw` (for both `formatversion=1` and `2`).
	...
	params?: unknown[];
	/**
	 * Present when `formatversion=2` and `errorformat` is either `plaintext` or `wikitext`.
	 */
	text?: string;
	/**
	 * Present when `formatversion=2` and `errorformat=html`.
	 */
	html?: string;
}
